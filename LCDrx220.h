...
//  LCD関数
//                                          2014/02/25 H. Suzuki
//                                          2022/6/28 30 7/1 2bitづつ版    ito RX220用に
//***************************************************************
...
#define   LCD_DT      PORTA.PODR.BYTE      // LCDデータポートPA0,1,3,4:4bit
#define   LCD_RS      PORT0.PODR.BIT.B3    // LCD制御ポート RS
#define   LCD_E       PORT0.PODR.BIT.B5    // LCD制御ポート E
...
//  LCD_DT = ( LCD_DT & 0xf0 ) | ( x & 0x0f );  // データセット
  d = x & 0x0f;
  d = d << 1;	      // 1ビットシフト
  d = (d & 0x18) | (x & 0x03);    // 3ビット目を空けたデータ
  LCD_DT = ( LCD_DT & 0xe0 ) | ( d & 0x1f );  // データセット
  LCD_E = 1; LCD_E = 0;                       // 送信
}
...
//  LCD_DT = ( LCD_DT & 0xf0 ) | ( ( x >> 4 ) & 0x0f ); // データセット
  d = ( x >> 4 ) & 0x0f;
  d = d << 1;	      // 1ビットシフト
  d = (d & 0x18) | ( ( x >> 4 ) & 0x03);    // 3ビット目を空けたデータ
  LCD_DT = ( LCD_DT & 0xe0 ) | ( d & 0x1f );  // データセット
...
//--------------------------------------------------------------------------
//  LCD_wait : LCD用wait関数
//    使用例 : LCD_wait( 数値 ) 6/30 ループを2倍に 7/1戻す
//--------------------------------------------------------------------------
void LCD_wait( int n )
{
  volatile int x;
