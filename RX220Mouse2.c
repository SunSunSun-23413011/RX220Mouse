...
void main(void)
{
  ...
  while( 1 ){
    ...
    if( MODE == 0 )
    {
      LCD_dec_out(  3, F_SEN, 4 ); // 前センサ値をLCD上中央に表示
...
void load_param( void ) 
{
  int i;
  // センサしきい値の決め打ち
  R_REF   = 506;    // 区画中央での右センサ値 11/6[147-459-537,317] 400
  L_REF   = 580;    // 区画中央での左センサ値 11/6[387-612-587,478] 550
  F_REF   = 1500;    // 区画中央での前センサ値 11/6[71-340-925] 250
  // 壁の有無判定用しきい値:各センサ壁あり最小値と壁なし値の中間値
  R_LIM   =  180;    // 右 11/6[147-459-537,317] 350
  ...
  // 走行パラメータ  // 1-2相励磁
    ...
    slalom_step_forward_table[gspeed_index_for_value(500)] = 8;  // 速度500用
    slalom_step_out_table[gspeed_index_for_value(500)] = 30; // 速度500用
